...
mongoose.connect('mongodb+srv://shyam:shyamkaviya@cluster0.u5jk0.mongodb.net/codeStorage?retryWrites=true&w=majority')
...
const snippetSchema = new mongoose.Schema({
    ...
    language: {
        type: String,
        required: true,
        enum: ['text', 'python', 'javascript', 'java', 'c', 'cpp', 'sql', 'html', 'css']
    },
    email: {
        type: String,
        required: true
...
app.post("/addCode", async (req, res) => {
    try {
        ...
        const newSnippet = new Snippet({ title, language, code, email });
...
app.get("/addCode", (req, res) => {
    ...
    res.render("addCode", { email: email || "" }); // Pass the email to the view
});
...
app.get('/snippet/:id', async (req, res) => {
    ...
    res.render('snippetDetail', { snippet });
});
